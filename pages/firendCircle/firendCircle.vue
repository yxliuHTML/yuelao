	...
	<view class="content" id="content">
		...
		<view class="content-circle">
			<view class="content-circle-box" v-for="(item, index) in circleData" :key="item.circleMegId">
				<view class="headimg">
					<image class="img" :src="item.userHeadImg" @tap="linkToBusinessCard(item.userId)"></image>
				</view>
				<view class="content">
					...
					<view class="content-img" v-if="item.imageList.length">
						...
						<view v-if="item.imageList.length == 1">
							<image class="img-1" :src="item.imageList[0]" mode="widthFix" @tap="previewImg(0, item.imageList)"></image>
						</view>
						<!-- 有多张图的时候 -->
						...
						<view class="icon-box">
							<view @tap="clickThumb(item)">
								<image class="img icon-box-item thumb"
									:src="item.isPraise ? require('@/static/like-fill.png') : require('@/static/like.png')" mode=""></image>
							</view>
					...
					<view class="msg-box">
						...
						<view class="comment" v-if="item.comment.length">
							<view class="comment-box" v-for="(comment, index) in item.comment" :key="index"
								hover-class="comment-hover-class" :id="`comment-${item.circleMegId}-${index}`"
		...
		<view class="input-box" v-if="showInput" id="input-box"
			...
			<u-action-sheet :list="list" v-model="show" border-radius="25" safe-area-inset-bottom
