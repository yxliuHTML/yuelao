...
<template>
	<view class="content">
		<!-- #ifdef MP-WEIXIN -->
		<u-navbar title=" " :background="{ background: '#f8f8f8' }" :border-bottom="false" back-icon-name="" back-text="取消"
			:back-text-style="{ color: '#404133' }">
		...
		<!-- #endif -->
		<textarea class="input" v-model="content" placeholder="这一刻的想法..." :show-confirm-bar="false" :adjust-position="false"
			:disable-default-padding="true"></textarea>
		<u-upload class="upload" ref="upload" :action="action" :auto-upload="false" :max-size="maxSize" :max-count="3"
			:size-type="['compressed']" @on-oversize="oversizeUpload"></u-upload>
		<!-- <view class="tips">
			<u-cell-group>
